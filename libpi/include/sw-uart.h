...
#define sw_uart_init(tx, rx, baud)                                             \
  sw_uart_init_helper(tx, rx, baud, BAUD_TO_CYCLES(baud), BAUD_TO_USEC(baud))

// create config equal to the default HW setup.
// pin 14 for TX
// pin 15 for RX
// baud = 115200
static inline sw_uart_t sw_uart_default(void) {
...
static inline void sw_uart_putk(sw_uart_t *s, const char *msg) {
  assert(msg);
...
static inline void sw_uart_printk(sw_uart_t *s, const char *fmt, ...) {
  char buf[1024];
  va_list args;
