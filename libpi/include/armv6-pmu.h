...
cp_asm(pmu_cycle, p15, 0, c15, c12, 1) cp_asm(pmu_event0, p15, 0, c15, c12, 2)
...
// makes any sense.
#define pmu_stmt_measure_set(cnt0, cnt1, msg, type0, type1, stmts)             \
  do {                                                                         \
    pmu_##type0##_on(0);                                                       \
    pmu_##type1##_on(1);                                                       \
    ...
    cnt1 = n_ty1;                                                              \
                                                                               \
    output("%s:%d: %s:\n\tcycles=%d\n\t%s=%d\n\t%s=%d\n", __FILE__, __LINE__,  \
           msg, n_cyc, s0, cnt0, s1, cnt1);                                    \
  } while (0)

#define pmu_stmt_measure(msg, type0, type1, stmts)                             \
  do {                                                                         \
...
#define XX(fn_name, type, val, string)                                         \
  /* enable event <type> */                                                    \
  static void pmu_##fn_name##_on(unsigned n) { pmu_enable(n, type); }          \
  /* read event <type> counter */                                              \
  static uint32_t pmu_##fn_name(unsigned n) { return pmu_event_get(n); }       \
  /* get printable string of event <type>*/                                    \
  static const char *pmu_##fn_name##_str(void) { return string; }
