...
static inline int mode_legal(unsigned mode) {
  if (mode == USER_MODE || mode == IRQ_MODE || mode == SUPER_MODE ||
      mode == ABORT_MODE || mode == UNDEF_MODE || mode == SYS_MODE)
    return 1;
...
static inline uint32_t cpsr_clear_carry(uint32_t cpsr) {
...
static inline int mode_eq(uint32_t mode) {
  return mode_get(cpsr_get()) == mode;
}
