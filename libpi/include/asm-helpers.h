...
#define cp_asm_set(fn_name, coproc, opcode_1, Crn, Crm, opcode_2)              \
  cp_asm_set_raw(fn_name, coproc, opcode_1, Crn, Crm,                          \
                 opcode_2) static inline void                                  \
  fn_name##_set(uint32_t v) {                                                  \
    fn_name##_set_raw(v);                                                      \
    prefetch_flush();                                                          \
  }

#define cp_asm_set_fn(fn_name, coproc, opcode_1, Crn, Crm, opcode_2)           \
  cp_asm_set_raw_fn(fn_name, coproc, opcode_1, Crn, Crm, opcode_2) void        \
  fn_name##_set(uint32_t v) {                                                  \
    fn_name##_set_raw(v);                                                      \
    prefetch_flush();                                                          \
  }
