...
static inline pretty_time_t pretty_time_mk(uint32_t start_usec) {
  uint32_t tot_usec = timer_get_usec() - start_usec;
  return (pretty_time_t){.sec = usec_to_sec(tot_usec),
                         .msec = usec_to_msec(tot_usec),
                         .usec = usec_to_usec(tot_usec)};
}

static inline void pretty_time_print(pretty_time_t t) {
  if (t.sec)
    output("%dsec:", t.sec);
  if (t.msec)
    output("%dms:", t.msec);
...
static inline void pretty_print_usec(uint32_t start_usec) {
