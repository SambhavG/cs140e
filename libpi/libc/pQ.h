// engler, cs140e: brain-dead generic queue. don't want to use STL/c++ in the
// kernel.
#ifndef __pQ_H__
#define __pQ_H__
#ifndef E
...
static int pQ_empty(pQ_t *q) { return (q->cnt == 0); }
...
static void pQ_insert(pQ_t *q, E *e, int priority) {
  ...
  while (1) {
    ...
    if (bubble_node > 1 &&
        q->stack[bubble_node].priority > q->stack[bubble_node / 2].priority) {
...
static heapElem_t pQ_pop(pQ_t *q) {
  ...
  while (1) {
    ...
    if (child1 <= q->cnt &&
        q->stack[child1].priority > q->stack[largest].priority) {
      largest = child1;
    }
    if (child2 <= q->cnt &&
        q->stack[child2].priority > q->stack[largest].priority) {
      largest = child2;
    }
...
static void pQ_init(pQ_t *q) { q->cnt = 0; }

static inline pQ_t pQ_mk(void) { return (pQ_t){0}; }
