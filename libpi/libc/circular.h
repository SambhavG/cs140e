...
//
// could simplify the interface if we wanted to only allow < 32bit valuesto be
// stored.
...
// XXX: i think change this so you can size the buffers to what you think makes
// sense. e.g., for output we probably want a huge one.  or dynamically adapt.
// if you want stack allocation they could pass in pointers to the buffer, etc.
//
// hardcoding N means we can stack allocate everything if needed.  (avoiding
// dynamic allocation seems like something worthwhile in an embedded system, but
// in our case perhaps is not relevant since we have the trivial kmalloc pointer
// bump.)
#ifndef CQ_N
#define CQ_N 8192
#endif
...
/*************************************************************************************
 * inline these so they can be used when we bit-bang off the UART with no
 * interrupts.
 */
...
static inline int cq_full(cq_t *q) {
  gcc_mb();
  return (q->head + 1) % (CQ_N) == q->tail;
}

static inline unsigned cq_nelem(cq_t *q) {
  gcc_mb();
  return (q->head - q->tail) % (CQ_N);
}
