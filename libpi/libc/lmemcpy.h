...
static inline void lmemset(void *dst, uint32_t v, size_t nbytes) {
  assert(dst);
  assert(nbytes % 4 == 0);
...
static inline void lmemcpy(void *dst, void *src, size_t nbytes) {
  assert(dst);
  assert(src);
  assert(nbytes % 4 == 0);
  assert(is_aligned4(dst));
...
static inline void llmemcpy(void *dst, void *src, size_t nbytes) {
  assert(dst);
  assert(src);
  assert(nbytes % 8 == 0);
