...
static inline uint32_t bit_not(uint32_t x, unsigned bit) {
  assert(bit < 32);
  return x ^ (1 << bit);
}
...
static inline uint32_t bits_set(uint32_t x, unsigned lb, unsigned ub,
                                uint32_t v) {
  assert(lb <= ub);
  assert(ub < 32);
...
// bits [lb:ub] == <val?
static inline unsigned bits_eq(uint32_t x, unsigned lb, unsigned ub,
                               uint32_t val) {
  assert(lb <= ub);
  assert(ub < 32);
...
static inline uint32_t bits_union(uint32_t x, uint32_t y) { return x | y; }

static inline uint32_t bits_intersect(uint32_t x, uint32_t y) { return x & y; }
static inline uint32_t bits_not(uint32_t x) { return ~x; }
