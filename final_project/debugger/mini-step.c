...
// Only the first breakpoint register will be used for single stepping
// (mismatch) The other five are used for regular breakpoints
...
static int bp_ctrl_is_enabled(unsigned index) {
  assert(index <= 5);
  if (index == 0 && cp14_bcr0_is_enabled())
    return 1;
  if (index == 1 && cp14_bcr1_is_enabled())
    return 1;
  if (index == 2 && cp14_bcr2_is_enabled())
    return 1;
  if (index == 3 && cp14_bcr3_is_enabled())
    return 1;
  if (index == 4 && cp14_bcr4_is_enabled())
    return 1;
  if (index == 5 && cp14_bcr5_is_enabled())
    return 1;
  return 0;
}
...
static unsigned bp_ctrl_get(unsigned index) {
  assert(index <= 5);
  if (index == 0)
    return cp14_bcr0_get();
  if (index == 1)
    return cp14_bcr1_get();
  if (index == 2)
    return cp14_bcr2_get();
  if (index == 3)
    return cp14_bcr3_get();
  if (index == 4)
    return cp14_bcr4_get();
  if (index == 5)
    return cp14_bcr5_get();
  assert(1 == 0);
}
...
static void bp_val_set(unsigned index, void *addr) {
  assert(index <= 5);
  if (index == 0)
    cp14_bvr0_set((unsigned)addr);
  if (index == 1)
    cp14_bvr1_set((unsigned)addr);
  if (index == 2)
    cp14_bvr2_set((unsigned)addr);
  if (index == 3)
    cp14_bvr3_set((unsigned)addr);
  if (index == 4)
    cp14_bvr4_set((unsigned)addr);
  if (index == 5)
    cp14_bvr5_set((unsigned)addr);
}
...
int mini_bp_is_breakpoint(void *addr) { return which_bp(addr) != -1; }

int mini_bp_enabled() { return num_bps; }

// called from exception handler: if the current
// fault is a watchpoint, return 1
int mini_bp_is_fault(void) { return was_brkpt_fault(); }

int mini_bp_num_faults() { return n_faults; }
