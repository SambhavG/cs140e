...
typedef struct {
  ...
  uint16_t
      backup_boot_loc;  // 50-51   Backup boot sector location or 0 or 0xffff
                        //         if none (usually 6)
  ...
  uint8_t reserved1;         // 65      Reserved - used to be Current Head
                             //      (used by Windows NT)
  uint8_t
      extended_sig;    // 66      Extended signature (0x29)
...
508-511 0xaa550000 - sector signature
 ...
 Is located at:
    uint16_t  info_sec_num;         // 48-49   Filesystem information sector
number in from start of the fat32 partition (see below)
*/
struct fsinfo {
  uint32_t sig1;               // 0-3
...
typedef struct {
  ...
  uint16_t
      mod_time; // 22-23   Time (5/6/5 bits, for hour/minutes/doubleseconds)
...
_Static_assert(sizeof(fat32_dirent_t) * NDIR_PER_SEC == 512,
               "NDIR_PER_SEC wrong");
