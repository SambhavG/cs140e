...
#define BUS_ADDRESSES_l2CACHE_ENABLED                                          \
  0x40000000 // Bus Addresses: disable_l2cache=0
#define BUS_ADDRESSES_l2CACHE_DISABLED                                         \
  0xC0000000 // Bus Addresses: disable_l2cache=1
...
#define MAIL_EMPTY                                                             \
  0x40000000 // Mailbox Status Register: Mailbox Empty (There is nothing to read
             // from the Mailbox)
#define MAIL_FULL                                                              \
...
#define Get_Max_Temperature                                                    \
  0x0003000A // Voltage: Get Max Temperature (Response: Temperature ID, Value In
             // Degrees C)
...
#define Get_Virtual_Buffer                                                     \
  0x00040004 // Frame Buffer: Get Virtual (Buffer) Width/Height (Response: Width
...
#define Test_Overscan                                                          \
  0x0004400A // Frame Buffer: Test Overscan (Response: Top In Pixels, Bottom In
             // Pixels, Left In Pixels, Right In Pixels)
#define Test_Palette                                                           \
  0x0004400B // Frame Buffer: Test Palette (Response: RGBA Palette Values (Index
             // 0 To 255))
...
#define Set_Virtual_Buffer                                                     \
  0x00048004 // Frame Buffer: Set Virtual (Buffer) Width/Height (Response: Width
             // In Pixels, Height In Pixels)
#define Set_Depth                                                              \
...
#define DMA_WAITING_FOR_OUTSTANDING_WRITES                                     \
  0x40 // DMA Control & Status: DMA is Waiting for the Last Write to be Received
...
#define PCM_TXTHR_0                                                            \
  0x0 // PCM Control & Status: Sets the TX FIFO Threshold at which point the TXW
      // flag is Set when the TX FIFO is Empty
#define PCM_TXTHR_1                                                            \
...
#define PCM_TXTHR                                                              \
  0x60 // PCM Control & Status: Sets the TX FIFO Threshold at which point the
       // TXW flag is Set
#define PCM_RXTHR_0                                                            \
  0x0 // PCM Control & Status: Sets the RX FIFO Threshold at which point the RXR
      // flag is Set when we have a single Sample in the RX FIFO
#define PCM_RXTHR_1                                                            \
...
#define PCM_CLKI                                                               \
  0x400000 // PCM Mode: Clock Invert this logically inverts the #define PCM_CLK
           // Signal
...
#define PCM_FLUSHED                                                            \
  0xFC00 // PCM Gray Mode Control: The Number of Bits that were Flushed into the
         // RX FIFO
