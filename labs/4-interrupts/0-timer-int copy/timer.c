...
static void timer_init(uint32_t prescale, uint32_t ncycles) {
  //**************************************************
  ...
  switch (prescale) {
  case 1:
    v = ARM_TIMER_CTRL_PRESCALE_1;
    break;
  case 16:
    v = ARM_TIMER_CTRL_PRESCALE_16;
    break;
  case 256:
    v = ARM_TIMER_CTRL_PRESCALE_256;
    break;
  default:
    panic("illegal prescale=%d\n", prescale);
  }

  // Q: if you change prescale?
  PUT32(ARM_Timer_Control, ARM_TIMER_CTRL_32BIT | ARM_TIMER_CTRL_ENABLE |
                               ARM_TIMER_CTRL_INT_ENABLE | v);
...
#define N 20
  ...
  unsigned tot = timer_get_usec() - start, tot_sec = tot / (1000 * 1000),
           tot_ms = (tot / 1000) % 1000, tot_usec = (tot % 1000);
  printk("-----------------------------------------\n");
  printk("summary:\n");
