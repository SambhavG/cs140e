...
// locations.
enum { REGS_SP = 13, REGS_LR = 14, REGS_PC = 15, REGS_CPSR = 16 };
...
static inline void mode_get_sp_lr(regs_t *r) {
  ...
  priv_get_sp_lr_asm(mode, &r->regs[REGS_SP], &r->regs[REGS_LR]);
}
...
static inline void __attribute__((noreturn)) switchto(regs_t *r) {
...
int switchto_save_regs(void (*fn)(uint32_t *regs, void *arg), uint32_t *regs,
                       void *arg);
...
static inline void switchto_cswitch(regs_t *old, regs_t *next) {
...
//
static inline regs_t switchto_mk(uint32_t fn, void *sp, uint32_t cpsr,
                                 void (*on_exit)(int retv)) {
  return (regs_t){.regs[REGS_PC] = fn,
