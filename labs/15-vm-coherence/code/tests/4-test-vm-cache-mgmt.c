...
// not in cache.
static void dcache_should_miss(const char *msg, volatile uint32_t *ptr) {
  uint32_t miss, cnt;
  ...
  pmu_stmt_measure_set(miss, cnt, msg, dcache_miss, inst_cnt, { *ptr; });
...
static void dcache_should_not_miss(const char *msg, volatile uint32_t *ptr) {
  uint32_t miss, access;

  // should not miss.
  pmu_stmt_measure_set(miss, access, "should be cached: should see no misses",
...
// (128/16=32).
static void icache_should_miss(const char *msg, uint32_t (*fp)(uint32_t)) {
  uint32_t miss, cnt;
  ...
  pmu_stmt_measure_set(miss, cnt, msg, icache_miss, inst_cnt, { fp(1); });
...
void notmain(void) {
  ...
  pmu_stmt_measure_set(base_miss, cnt, "testing miss on nop", icache_miss,
                       inst_cnt, { asm volatile("nop"); });
