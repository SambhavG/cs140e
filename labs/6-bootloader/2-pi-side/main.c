...
static inline int boot_has_data(void) { return uart_has_data(); }
...
static inline uint8_t boot_get8(void) { return uart_get8(); }
...
// can check by making sure you get a get_code() < 0
// (not reboot, not lockup) if there is an error.
static void boot_put8(uint8_t x) { uart_put8(x); }
