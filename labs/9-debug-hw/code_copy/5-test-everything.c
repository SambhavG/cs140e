...
//     printk("[Custom bp handler] Custom bp handler triggered: can see that the breakpoint was triggered at pc=%x\n", s->fault_pc);
//     printk("[Custom bp handler] The registers are: %x, %x, %x, %x, %x\n", s->regs->regs[0], s->regs->regs[1], s->regs->regs[2], s->regs->regs[3], s->regs->regs[4]);
// }

// static void wp_handler(void *data, watch_fault_t *w) {
//     printk("[Custom wp handler] Custom wp handler triggered: can see that the watchpoint was triggered at pc=%x\n", w->fault_pc);
//     printk("[Custom wp handler] The registers are: %x, %x, %x, %x, %x\n", w->regs->regs[0], w->regs->regs[1], w->regs->regs[2], w->regs->regs[3], w->regs->regs[4]);
// }
