...
typedef struct nrf {
  ...
  uint32_t start_usec, tot_sent_msgs, tot_sent_bytes,

      tot_retrans, tot_lost,

      tot_recv_msgs, tot_recv_bytes;
} nrf_t;
...
// obviously.
int nrf_tx_send_ack(nrf_t *n, uint32_t txaddr, const void *msg,
                    unsigned nbytes);
int staff_nrf_tx_send_ack(nrf_t *n, uint32_t txaddr, const void *msg,
                          unsigned nbytes);

// send <nbytes> of data pointed to by <msg> to <txaddr> using NRF <n>,
// no hardware ack.  the NRF must have been configured for no-ACKs.
int nrf_tx_send_noack(nrf_t *n, uint32_t txaddr, const void *msg,
                      unsigned nbytes);
int staff_nrf_tx_send_noack(nrf_t *n, uint32_t txaddr, const void *msg,
                            unsigned nbytes);
...
// using NRF <nic>
int nrf_send_noack(nrf_t *nic, uint32_t txaddr, const void *msg,
                   unsigned nbytes);
...
static inline int nrf_legal_Mhz(unsigned d) { return d < (1 << 8); }
...
static inline unsigned nrf_get_addr_nbytes(nrf_t *nic) {
  return nrf_default_addr_nbytes;
}

// should we pull in more configuration stuff?
static inline nrf_conf_t nrf_conf_mk(uint32_t nbytes, uint32_t ch) {
...
#define nrf_opt_assert(nic, cond)                                              \
  do {                                                                         \
    if (opt_check_p)                                                           \
      nrf_assert(nic, cond);                                                   \
  } while (0)
...
#define opt_put8_chk(_n, _reg, _v)                                             \
  do {                                                                         \
    if (opt_check_p)                                                           \
