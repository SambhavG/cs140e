...
#define nrf_put8_chk(nic, reg, v) nrf_put8_chk_helper(SRC_LOC_MK(), nic, reg, v)
...
// is NRF in TX mode?
static inline int nrf_is_tx(nrf_t *nic) { return nrf_is_rx(nic) == 0; }
...
static inline int pipeid_empty(unsigned pipeid) { return pipeid == 0b111; }
// extract pipeid from status value.
static inline int pipeid_get(uint8_t status) { return bits_get(status, 1, 3); }
...
static int nrf_rx_has_packet(nrf_t *nic) { return !nrf_rx_fifo_empty(nic); }
...
static inline int nrf_is_pwrdown(nrf_t *nic) { return !nrf_is_pwrup(nic); }
