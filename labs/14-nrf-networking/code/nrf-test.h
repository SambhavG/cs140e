...
static inline nrf_conf_t parthiv_left(uint32_t nbytes, uint32_t ch) {
...
static inline nrf_conf_t parthiv_right(uint32_t nbytes, uint32_t ch) {
  nrf_conf_t c = nrf_conf_mk(nbytes, ch);
  c.spi_chip = 1;
  c.ce_pin = 5;
  c.int_pin = 23;
...
static inline nrf_conf_t client_conf(uint32_t nbytes) {
  return parthiv_right(nbytes, nrf_default_channel);
}
// default: use parthiv left for server.
static inline nrf_conf_t server_conf(uint32_t nbytes) {
...
static inline nrf_t *server_mk(uint32_t rxaddr, uint32_t nbytes, int ack_p) {
  return nrf_init(server_conf(nbytes), rxaddr, ack_p);
}
static inline nrf_t *server_mk_ack(uint32_t rxaddr, uint32_t nbytes) {
  return server_mk(rxaddr, nbytes, 1);
}
static inline nrf_t *server_mk_noack(uint32_t rxaddr, uint32_t nbytes) {
  return server_mk(rxaddr, nbytes, 0);
}

// same as server, but for clients.
static inline nrf_t *client_mk(uint32_t rxaddr, uint32_t nbytes, int ack_p) {
  return nrf_init(client_conf(nbytes), rxaddr, ack_p);
}
static inline nrf_t *client_mk_ack(uint32_t rxaddr, uint32_t nbytes) {
  return client_mk(rxaddr, nbytes, 1);
}
static inline nrf_t *client_mk_noack(uint32_t rxaddr, uint32_t nbytes) {
