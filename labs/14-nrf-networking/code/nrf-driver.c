...
nrf_t *nrf_init(nrf_conf_t c, uint32_t rxaddr, unsigned acked_p) {
  ...
  if (!acked_p) {
    ...
    nrf_put8_chk(n, NRF_SETUP_RETR,
                 (nrf_default_retran_delay / 250 - 1) << 4 |
                     nrf_default_retran_attempts);
  ...
  nrf_put8_chk(n, NRF_EN_RXADDR, nrf_get8(n, NRF_EN_RXADDR) & 0b11);
  ...
  // clear interrupts
  nrf_put8(n, NRF_STATUS, nrf_get8(n, NRF_STATUS));
  ...
  nrf_put8(n, NRF_CONFIG, nrf_get8(n, NRF_CONFIG) | 0b10);
  delay_ms(2);
...
int nrf_tx_send_ack(nrf_t *n, uint32_t txaddr, const void *msg,
  ...
  // Wait for interrupt
  while (1) {
    if (nrf_get8(n, NRF_STATUS) & (1 << 5))
      break;

    // Check for max resends
    if (nrf_has_max_rt_intr(n))
      panic("Max interrupts!\n");
  }
  uint8_t cnt = nrf_get8(n, NRF_OBSERVE_TX);
  n->tot_retrans += bits_get(cnt, 0, 3);
...
int nrf_get_pkts(nrf_t *n) {
  ...
  while (!nrf_rx_fifo_empty(n)) {
    assert(nrf_rx_get_pipeid(n) != (uint32_t)pipeid_empty);
    char buf[32];
