...
void nrf_dump(const char *msg, nrf_t *nic) {
  ...
  nrf_output("\tCONFIG (%b):\tPWR_UP=%d, PRIM_RX=%d, CRC=%d EN_CRC=%d", x,
             bit_get(x, 1), bit_get(x, 0), bit_get(x, 2), bit_get(x, 3));
  output(" MASK_RX_DR==%d, MASK_TX_DS=%d, MASK_MAX_RT=%d\n", bit_get(x, 6),
         bit_get(x, 5), bit_get(x, 4));
  ...
  nrf_output("\tSTATUS(=%b): RX-int=%d, TX-int=%d, RT-int=%d RXpipe=%b (%s), "
             "TX-full=%d\n",
             x, bit_get(x, 6), bit_get(x, 5), bit_get(x, 4), bits_get(x, 1, 3),
  ...
  nrf_output(
      "\tFIFO_STATUS(=%b): TX_FULL=%d, TX_EMPTY=%d, RX_FULL=%d, RX_EMPTY=%d\n",
      x, bit_get(x, 5), bit_get(x, 4), bit_get(x, 1), bit_get(x, 0));
...
static inline frac_t kbytes_per_sec(unsigned tot_bytes, unsigned tot_usec) {
