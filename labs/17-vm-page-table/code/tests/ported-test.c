...
static void data_abort_handler(regs_t *r) {
  ...
  if (reason == 0b1001) {
    ...
    printk("Other fault: %x\n", reason);
  }

  // Reenable dom perms
  uint32_t dom_perms = dacr_get(); // staff_domain_access_ctrl_get();
  dom_perms = dom_perms | (0b11 << 7 * 2);
  // Print ifsr
  // if (num_faults == 3) {
  ...
  //     clean_reboot();
  // }
  printk("Giving back dom perms. New dom perms: %x\n", dom_perms);
...
void notmain(void) {
  ...
  // Test fault handler with domain permissions

  // Turn off domain perms for dom_fault
  uint32_t dom_perms = dacr_get(); // staff_domain_access_ctrl_get();
  printk("Old dom perms: %x\n", dom_perms);
  dom_perms = dom_perms & ~(0b11 << dom_fault * 2);
  printk("New dom perms: %x\n", dom_perms);
  domain_access_ctrl_set(dom_perms);

  // load using get32 to heap
  uint32_t *addr = (uint32_t *)SEG_HEAP + 0xbeef; // addr in heap
  // vm_mmu_enable();
  uint32_t val = get32(addr); // should fault

  // same deal for put32

  dom_perms = dacr_get(); // staff_domain_access_ctrl_get();
  printk("Old dom perms: %x\n", dom_perms);
  dom_perms = dom_perms & ~(0b11 << dom_fault * 2);
  printk("New dom perms: %x\n", dom_perms);
  domain_access_ctrl_set(dom_perms);
  addr = (uint32_t *)SEG_HEAP + 0xbeef; // addr in heap
  put32(addr, 0xdeadbeef);              // should fault

  // now write bx lr to a heap location and jump to it

  // bx lr is e12fff1e
  addr = (uint32_t *)SEG_HEAP + 0xbeef; // addr in heap
  printk("Writing bx lr to addr %x\n", addr);
  put32(addr, 0xe12fff1e);

  dom_perms = dacr_get(); // staff_domain_access_ctrl_get();
  printk("Old dom perms: %x\n", dom_perms);
  dom_perms = dom_perms & ~(0b01 << dom_fault * 2);
  printk("New dom perms: %x\n", dom_perms);
  domain_access_ctrl_set(dom_perms);
  // set pc to addr
  //  asm volatile ("blx %0":: "r" (addr));
  void (*fn)(void) = (void (*)(void))addr;
  printk("value at addr: %x\n", *addr);
  printk("VVVVV\n");
  fn();
  printk("^^^^^\n");

  printk("Back from exception\n");
}
