/*
   ...
   problem: the ARM eabi specifies for *some* of the registes which are caller
   and which are callee-saved.  for others, it allows either to hold.  it's
   unclear in general what gcc does (and it appears it can make different
   choices depending on options).

   so, our problem: for the specific gcc and specific compiler flags, which
   polarity are different registers?
   ...
   caller-saved (barring a compiler bug).  however, the others might or might
   not be callee-saved because redundant saving isn't incorrect, just slow.  so
   we rely on high compiler optimization.   also it does appear that it handles
   lr weirdly.
   ...
   registers r4-r8 are callee-save registers *r9 might be a callee-save register
   or not (on some variants of AAPCS it is a special register) r10-r11 are
   callee-save registers *r12-r15 are special registers --- r12 seems to be a
   caller saved.
