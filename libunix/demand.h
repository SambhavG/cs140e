...
#define impossible(args...)                                                    \
  do {                                                                         \
    fprintf(stderr, "%s:%s:%d:IMPOSSIBLE ERROR:", __FILE__, __FUNCTION__,      \
            __LINE__);                                                         \
    fprintf(stderr, ##args);                                                   \
    exit(1);                                                                   \
  } while (0)
...
#define demand(_expr, _msg, args...)                                           \
  do {                                                                         \
    if (!(_expr)) {                                                            \
      fprintf(stderr, "%s:%s:%d: Assertion '%s' failed:", __FILE__,            \
              __FUNCTION__, __LINE__, _XSTRING(_expr));                        \
...
#define sys_die(syscall, msg, args...)                                         \
  do {                                                                         \
...
#define sys_debug(syscall, msg)                                                \
  do {                                                                         \
    fprintf(stderr, "%s:%s:%d: <%s>\n\tperror=", __FILE__, __FUNCTION__,       \
            __LINE__, _XSTRING(msg));                                          \
    perror(_XSTRING(syscall));                                                 \
  } while (0)

// the called code cannot fail (<0)
#define no_fail(syscall)                                                       \
  do {                                                                         \
    if ((syscall) < 0) {                                                       \
      int reason = errno;                                                      \
      sys_die(syscall,                                                         \
